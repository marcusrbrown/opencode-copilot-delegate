...
- **JSON body parsing.** `node:http` does not parse request bodies. The Unit 3 server collects `data`/`end` chunks per request, parses with `JSON.parse` inside try/catch, then validates against the Zod schema. Bad bodies → 400.
...
- **Spawn toast helper.** Unit 4 adds `notifySpawn(client, taskId)` in `src/runtime/notify.ts` parallel to existing `notifyCompletion`, using the same `client.tui.showToast({ body: { message, variant } })` shape. Variant is always `info`.
...
- *Error path:* `POST /tasks/cancel` without `Authorization` header returns 401.
- *Error path:* `POST /tasks/cancel` with wrong token returns 401 (verify `timingSafeEqual` — same response time for valid-format-wrong-value as for malformed).
...
- *Error path:* Request body exceeds 64KB cap returns 413.
