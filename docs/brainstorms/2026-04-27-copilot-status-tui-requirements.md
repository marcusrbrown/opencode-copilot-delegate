...
- **PID-file workaround edge cases.** PID reuse across reboots, files left behind by SIGKILL, races between concurrent plugin loads. Mitigation: include process start time alongside PID in the file (compare against `/proc` or `ps` to detect reuse); always wrap reap in try/catch; treat the file as advisory not authoritative.
...
- The `tool_calls_summary` data already in `src/runtime/envelope.ts` is the right input to the v0.2.0 list view's tool-call count column.
